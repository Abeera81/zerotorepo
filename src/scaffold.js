 ...
 */
function generateScaffoldFiles(projectName, description, research, startupName) {
  const displayName = startupName?.name || projectName;
  const tagline = startupName?.tagline || description || 'A new project scaffolded by ZeroToRepo.';
  ...
  // Build a rich README with research
  let readme = `# ${displayName}\n\n`;
  readme += `> ${tagline}\n\n`;

  if (startupName?.reasoning) {
    readme += `*${startupName.reasoning}*\n\n`;
  }

  // Problem & Description
  readme += `## 💡 The Problem\n\n`;
  readme += `${description || 'Solving a real problem in an innovative way.'}\n\n`;

  // Competitive Landscape
  if (research?.competitors?.length > 0) {
    readme += `## 🏆 Competitive Landscape\n\n`;
    readme += `| Competitor | Pricing | Strengths | Weaknesses |\n`;
    readme += `|---|---|---|---|\n`;
    for (const c of research.competitors) {
      const strengths = (c.strengths || []).join(', ') || '—';
      const weaknesses = (c.weaknesses || []).join(', ') || '—';
      readme += `| [${c.name}](${c.url || '#'}) | ${c.pricing || 'Unknown'} | ${strengths} | ${weaknesses} |\n`;
    }
    readme += '\n';
  }
  ...
  if (research?.gaps?.length > 0) {
    readme += `## 🔍 Key Gaps & Our Opportunity\n\n`;
    for (const gap of research.gaps) {
      const badge = gap.severity === 'high' ? '🔴' : gap.severity === 'medium' ? '🟡' : '🟢';
      readme += `- ${badge} **${gap.gap}** — ${gap.opportunity}\n`;
    }
    readme += '\n';
  }
  ...
  if (research?.marketInsights) {
    ...
    readme += `## 📊 Market Insights\n\n`;
    if (mi.targetAudience) readme += `- **Target Audience:** ${mi.targetAudience}\n`;
    if (mi.marketSize) readme += `- **Market Size:** ${mi.marketSize}\n`;
    if (mi.trends?.length > 0) readme += `- **Trends:** ${mi.trends.join(', ')}\n`;
    readme += '\n';
  }

  // Tech Recommendations
  if (research?.techRecommendations?.length > 0) {
    readme += `## 🛠️ Recommended Tech Stack\n\n`;
  ...
  readme += `---\n*Auto-generated by [ZeroToRepo](https://github.com/zerotorepo) — Idea to GitHub in seconds.*\n`;
  ...
  return [
    ...
    {
      path: 'src/index.js',
      content: `// ${displayName} — ${tagline}\nconsole.log('Welcome to ${displayName}!');\n`,
    },
  ];
}
